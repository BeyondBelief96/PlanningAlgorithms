...
**Step 8, last rung** · **Guide:** [04-merging.md](04-merging.md)

## Implement

```ts
planHybridAStar(
  cspace: ZoneLayer, start: Pose, candidate: MergeCandidate,
  allowedZones: readonly ZoneClass[], params: HybridAStarParams
): MergePath
```

This is [Part 1's Problem 07](../taxi/p07-big-chart.md) again — the same priority
queue, the same settled-state bookkeeping, the same estimate that never runs high
— run over poses instead of chart points. Two things make it a *motion* planner:
...
estimate     Euclidean distance to the goal
settled set  a lattice: params.positionResolution metres, params.headingBins bins
validity     every sample of a primitive is in allowedZones, on the C-space layer
```

Poses are continuous, so Part 1's "settled" set becomes a lattice: keep the best
cost seen per `(cell x, cell y, heading bin)`. Euclidean distance never
...
clear, you are done and the path lands exactly on the merge pose.

This is the standard trick, and the reason it works is worth stating: the
estimate guides A\* toward the goal, and the analytic expansion is a cheap test
for "is the remaining problem now easy?" applied to every node the estimate
...
**`turnPenalty ≥ 1` keeps the estimate admissible.** Making arcs cheaper than
straights would not.
...
**Voronoi or obstacle-distance estimate.** Euclidean distance is very weak in a
