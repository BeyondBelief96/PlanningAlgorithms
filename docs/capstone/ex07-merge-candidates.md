...
**Step 7** · **Guide:** [04-merging.md](04-merging.md)

## Implement

```ts
generateMergeCandidates(
  gated, layer, aircraft, localization, permissions, filter, costToGo
): MergeCandidate[]
```
...
- **Allowed and reachable.** `allowsEdge(filter, d)`, then
  `k = advanceRouteIndex(edge, 0, routeLabels)` — negative means the clearance
  does not let you join this taxiway *first* — then
  `costToGoReachable(costToGo, d, k)`.
- **Junction exclusion.** `JUNCTION_EXCLUSION` clear of both ends.
- **Lead-in.** `MIN_LEAD_IN` metres of straight guidance line after the merge
  point, following collinear continuations across junctions.
- **Range and bearing.** Within `CANDIDATE_RADIUS`, and within
  `MAX_CANDIDATE_BEARING` of the nose — both in *where* the candidate is and in
