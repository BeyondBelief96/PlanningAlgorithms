...
**Step 2** · **Guide:** [docs/capstone/01-zone-layer.md](../../../docs/capstone/01-zone-layer.md)
...
1. **Copy the raw vertices.** Unchanged, same order, so their ids still mean what
   the chart meant.
2. **Split every edge where the zone changes.** Walk it in `kZoneScanStep` steps
   asking `zoneAt()`; where the answer changes, bisect to `kZoneSplitTolerance`
   and put a vertex there. Give each sub-edge the zone of its own midpoint, and
   carry `oneWay`, `maxWingspan` and `maxWeightTonnes` over from the parent.
...
enum order. That pair is what makes Exercise 05's gate check directional, and
"directional" is what lets an aeroplane leave a runway without a clearance.
...
```powershell
ctest --test-dir build/vs -C Debug -R capstone.ex02 --output-on-failure
```
...
```powershell
./build/vs/Debug/taxi_demo.exe gates
```
