...
**Step 1** · **Guide:** [docs/capstone/01-zone-layer.md](../../../docs/capstone/01-zone-layer.md)

## Implement

```cpp
ZoneQuery zoneAt(const ZoneLayer& layer, const Vec2& p);
FootprintZones classifyFootprint(const ZoneLayer&, const AircraftModel&, const Pose&);
ZoneLayer configurationSpace(const ZoneLayer&, const AircraftModel&);
```
...
```powershell
ctest --test-dir build/vs -C Debug -R capstone.ex01 --output-on-failure
```
...
enough that the answer is interesting, and `validateSweep()` plus `planHybridAStar()`
will show the difference straight away.
