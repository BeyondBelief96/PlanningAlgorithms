...
**Step 8** · **Guide:** [docs/capstone/04-merging.md](../../../docs/capstone/04-merging.md)

## Implement

```cpp
const char* toString(MergeMethod);
MergeParams paramsFor(StartMode, const AircraftModel&);

MergePath planCaptureWindow   (const Pose&, const MergeCandidate&, const MergeParams&);
MergePath planStraightThenTurn(const Pose&, const MergeCandidate&, const MergeParams&);
MergePath planSCurve          (const Pose&, const MergeCandidate&, const MergeParams&);
MergePath planIntercept       (const Pose&, const MergeCandidate&, const MergeParams&);
MergePath planDubinsMerge     (const Pose&, const MergeCandidate&, const MergeParams&);
MergePath planMerge           (const Pose&, const MergeCandidate&, const MergeParams&);
```
...
candidate from two poses, so it passes on its own, in any order.
...
**5. Dubins.** `dubinsShortestPath()` is given (see `geometry.hpp`) because it is
Section 15.3.1 of the book. What *is* yours is trying the preferred radius then
the minimum, and applying the loop-rejection filter.
...
**The S-curve exists because the SC solver fails.** Six metres off the centreline
and eight degrees out, the straight-then-turn answer wants to reverse two hundred
metres first. That is not a bug in your solver; it is the geometry, and it is
what rung 3 is for. The test asserts both halves of that.
...
twice; a taxi manoeuvre may not. `path.totalTurning() > params.maxTotalTurning`
is the test, and it is what makes `planMerge()` refuse a merge point five metres
...
```powershell
ctest --test-dir build/vs -C Debug -R capstone.ex08 --output-on-failure
```
...
- The S-curve solves the case the SC solver cannot, respects the minimum radius,
  and is what `planMerge()` picks — it does not fall through to Dubins.
...
- Dubins lands exactly, and **a merge point five metres behind is refused**
  rather than looped.
- The ladder picks capture, then SC, then S-curve, on three cases designed to
  land on each rung.

## Once it is green

```powershell
./build/vs/Debug/taxi_demo.exe taxiway-capture
...
`src/airport/geometry.cpp` does it that way, including the self-check, so read it
