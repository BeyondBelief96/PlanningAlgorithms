...
**Step 6** · **Guide:** [docs/capstone/03-graph-search.md](../../../docs/capstone/03-graph-search.md)

## Implement

```cpp
double zoneCostRate(ZoneClass zone);
double edgeCost(const TaxiGraph&, const DirectedEdge&, const AircraftModel&);
double partialEdgeCost(const TaxiGraph&, const DirectedEdge&, double s, const AircraftModel&);
int advanceRouteIndex(const Edge&, int k, const std::vector<std::string>& labels);
double CostToGo::at(const DirectedEdge&, int k) const;
bool CostToGo::reachable(const DirectedEdge&, int k) const;
CostToGo computeCostToGo(const TaxiGraph&, const PermissionSet&, const EdgeFilter&,
                         const AircraftModel&);
...
product graph rather than a filter applied afterwards. The difference matters: as
a filter, the cheapest unconstrained route usually violates the clearance, so you
reject everything and have no answer.
...
- **Relax:** for state `(d, k)`, look at every allowed edge `p` *arriving* at
  ...
  `advanceRouteIndex(edge(d), kp, labels)` must equal `k`. Then
  `V(p, kp) ← V(d, k) + edgeCost(d)`.

Record `CostToGo::next` as you relax, so `extractGraphRoute()` can walk the
answer out.
...
zone is runway or runway protected   -> k        transparent
...
**Fixed penalties do not prorate.** `kHotspotPenalty` and `kSoftGatePenalty` are
...
`edgeCost()` as `partialEdgeCost(..., 0.0, ...)` and there is only one of them to
get wrong.
...
**Do not expect `reachable()` to mean "forward reachable".** `computeCostToGo()`
will happily put a finite number on a state the aircraft could never be in. The
cost *from* a state is well defined whether or not you can get there, and forward
reachability is enforced in Exercise 07, where the candidate picks its starting
route index.
...
```powershell
./build/vs/Debug/taxi_demo.exe stand-departure
```
...
This is Dijkstra; the goal is a single vertex and the heuristic is free. Make it
A\* over the product space and work out what an admissible heuristic looks like
