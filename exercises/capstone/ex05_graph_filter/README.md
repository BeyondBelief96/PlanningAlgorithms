...
**Step 5** · **Guide:** [docs/capstone/03-graph-search.md](../../../docs/capstone/03-graph-search.md)

## Implement

```cpp
EdgeFilter filterGraph(const TaxiGraph& gated, const AircraftModel&, const PermissionSet&);
bool turnIsFeasible(const TaxiGraph&, const DirectedEdge& in, const DirectedEdge& out,
                    const AircraftModel&);
```
...
the holding position and the runway edge is twenty-five metres long, and that is
what decides whether a given aircraft can use it.

**A reversal is not a turn.** Refuse when the outgoing edge is the incoming one
reversed, and refuse when the turn exceeds `kMaxNodeTurn`. That single line is
...
```powershell
ctest --test-dir build/vs -C Debug -R capstone.ex05 --output-on-failure
```
...
gate the controller has just closed. Add a `std::vector<Aabb> blockages`
parameter and strike out the edges they cover, then think about what
`shouldReplan()` in Exercise 12 has to do when one appears.
