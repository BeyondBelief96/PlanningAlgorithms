...
**Step 10** · **Guide:** [docs/capstone/06-route-and-monitor.md](../../../docs/capstone/06-route-and-monitor.md)

## Implement

```cpp
Route assembleRoute(const TaxiGraph& gated, const ZoneLayer&, const AircraftModel&,
                    const MergePath&, const std::vector<DirectedEdge>& graphRoute,
                    const PermissionSet&, const SpeedLimits& = SpeedLimits{});
```
...
`alongTrack` of the nose tip in the frame of the gate. Forty iterations is exact
...
```powershell
ctest --test-dir build/vs -C Debug -R capstone.ex11 --output-on-failure
```
