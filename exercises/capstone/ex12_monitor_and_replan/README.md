...
**Steps 11 and 12** · **Guide:** [docs/capstone/06-route-and-monitor.md](../../../docs/capstone/06-route-and-monitor.md)

## Implement

```cpp
MonitorReport geofenceMonitor(const ZoneLayer&, const AircraftModel&, const VehicleState&,
                              const PermissionSet&, double horizonSeconds = 6.0,
                              double stepSeconds = 0.25);
ReplanDecision shouldReplan(const ReplanTriggers&, double speed, double crossTrackLimit = 3.0,
                            double periodSeconds = 5.0, double commitSeconds = 3.0);
...
```powershell
ctest --test-dir build/vs -C Debug -R capstone.ex12 --output-on-failure
```
...
```powershell
./build/vs/Debug/taxi_demo.exe
```

Eight scenarios. Five succeed, three refuse, and the three refuse for three
different reasons. **A planner that returns a path for all eight is worse than
one that returns a path for five** — that is the last idea in this capstone and
the one most worth taking away.
