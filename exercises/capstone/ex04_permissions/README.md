...
**Step 4** · **Guide:** [docs/capstone/02-localization-and-clearance.md](../../../docs/capstone/02-localization-and-clearance.md)

## Implement

```cpp
PermissionSet buildPermissions(const TaxiGraph& gated, const ZoneLayer&, const Clearance&);
```
...
`parseClearance()` is given — turning words into a struct is not planning. What
you build from the struct:
...
```powershell
ctest --test-dir build/vs -C Debug -R capstone.ex04 --output-on-failure
```
...
Try feeding the demo a different clearance by editing `scenarios::standDeparture()`,
or read `parseClearance()` and see which phrasings it understands. "Cross 36"
works as well as "cross runway 36"; "hold short of runway 27" as well as "hold
short runway 27".

## Extension

Real clearances are read back, amended, and superseded. Add `amend(const
PermissionSet&, const Clearance&)`, work out what an amendment means when the
